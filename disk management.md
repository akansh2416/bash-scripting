...
Before creating or mounting anything, check what block devices exist:
...
When to Use fdisk
Use fdisk when the disk is brand new and has no partitions.
...
lsblk
When to Use mount
Use mount when the partition already exists and is formatted.
...
Use fdisk + mkfs + mount when the disk is completely new: partition → format → mount.
